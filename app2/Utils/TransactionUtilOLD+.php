...
//        $output['subtotal'] = ($transaction->total_before_tax != 0) ? $this->num_f($transaction->total_before_tax, $show_currency) : 0;
	    $iraqiCurrency = Currency::where('country', '=', 'Iraq')->first();
	    $subtotal = $transaction->total_before_tax * $iraqiCurrency->currency_rate;
	    $output['subtotal'] = $iraqiCurrency->symbol.' '.(($transaction->total_before_tax != 0) ? $this->num_f($subtotal) : 0);
...
//            $output['total'] = $this->num_f($transaction->final_total, $show_currency);
	        $iraqiCurrency = Currency::where('country', '=', 'Iraq')->first();
	        $total = $transaction->final_total * $iraqiCurrency->currency_rate;
	        $output['total'] = $total;
        }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $is_product_expiry_enabled, $is_lot_number_enabled)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'unit_price_inc_tax' => $iraqiCurrency->symbol.' '.$this->num_f($line->unit_price_inc_tax * $iraqiCurrency->currency_rate),
                'unit_price_exc_tax' => $this->num_f($line->unit_price),

                //Fields for 4th column
                'line_total' => $iraqiCurrency->symbol.' '.$this->num_f($line->unit_price_inc_tax * $line->quantity * $iraqiCurrency->currency_rate),
            ];
