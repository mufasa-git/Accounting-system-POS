...
//                'tax_id' => $product['tax_id'],
     ...
     * @return array
     */
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type, $print_currency = null)
    {
        $il = $invoice_layout;
...
//             $output['invoice_date'] = \Carbon::createFromFormat('Y-m-d H:i:s', $transaction->transaction_date)->format('M d, Y H:i');
        ...
        if ($transaction_type == 'sell') {
            ...
            $details = $this->_receiptDetailsSellLines($lines, $il, $is_product_expiry_enabled, $is_lot_number_enabled, $print_currency);
	    ...
	    $print_currency_rate = Currency::where('country', '=', 'USA')->first();
	    if ($print_currency == "USD") {
		    $print_currency_rate = Currency::where('country', '=', 'USA')->first();
	    } else if ($print_currency == "IQD") {
		    $print_currency_rate = Currency::where('country', '=', 'Iraq')->first();
	    }
...
//        $output['subtotal'] = ($transaction->total_before_tax != 0) ? $this->num_f($transaction->total_before_tax, $show_currency) : 0;
	    $subtotal = $transaction->total_before_tax * $print_currency_rate->currency_rate;
	    $output['subtotal'] = $print_currency_rate->symbol.' '.(($transaction->total_before_tax != 0) ? $this->num_f($subtotal) : 0);
...
//            $output['total'] = $this->num_f($transaction->final_total, $show_currency);
	        $total = $transaction->final_total * $print_currency_rate->currency_rate;
	        $output['total'] = $total;
        }
...
//            $output['total_due'] = ($due == 0) ? 0 : $this->num_f($due, $show_currency);
     ...
     * @return array
     */
    protected function _receiptDetailsSellLines($lines, $il, $is_product_expiry_enabled, $is_lot_number_enabled, $print_currency=null)
    {
	    $print_currency_rate = Currency::where('country', '=', 'USA')->first();
	    if ($print_currency == "USD") {
		    $print_currency_rate = Currency::where('country', '=', 'USA')->first();
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'unit_price_inc_tax' => $print_currency_rate->symbol.' '.$this->num_f($line->unit_price_inc_tax * $print_currency_rate->currency_rate),
                'unit_price_exc_tax' => $this->num_f($line->unit_price),

                //Fields for 4th column
                'line_total' => $print_currency_rate->symbol.' '.$this->num_f($line->unit_price_inc_tax * $line->quantity * $print_currency_rate->currency_rate),
            ];
